# md2pdf - Convert clipboard markdown content to epub and pdf
...
md2pdf::process_markdown() {
    local tmp_dir="$1"
    local tmp_md="${tmp_dir}/md2pdf.md"
...
md2pdf::process_links() {
    ...
    while IFS= read -r line; do
        ...
        while [[ "$modified_line" =~ '\[([^]]+)\]\(([^)]+)\)' ]]; do
            ...
            # Add to references
            echo "$counter. [$link_text]($link_url)↗" >> "$references"
...
md2pdf::create_latex_pdf() {
    local tmp_dir="$1"
    local tmp_md="${tmp_dir}/md2pdf.md"
    local tmp_latex="${tmp_dir}/md2pdf.tex"
    local title="$2"
    local author="$3"
    
    echo "Converting markdown to LaTeX..."
    
    # Create LaTeX preamble with title and author
    md2pdf::create_latex_preamble "$tmp_dir" "$title" "$author"
...
EOF2
    ...
    cat "$tmp_latex" > "${tmp_dir}/md2pdf_temp.tex"
    
    # Generate PDF
    md2pdf::generate_pdf "$tmp_dir"
}

# Generate PDF from LaTeX
md2pdf::generate_pdf() {
    local tmp_dir="$1"
    local tmp_pdf="${tmp_dir}/md2pdf.pdf"
    
    echo "Converting LaTeX to PDF..."
    
    # Run xelatex twice to resolve references properly
    xelatex -interaction=nonstopmode -output-directory="$tmp_dir" "${tmp_dir}/md2pdf_temp.tex" > "${tmp_dir}/xelatex_log.txt" 2>&1
    xelatex -interaction=nonstopmode -output-directory="$tmp_dir" "${tmp_dir}/md2pdf_temp.tex" >> "${tmp_dir}/xelatex_log.txt" 2>&1

    # Ensure the PDF is correctly named
    if [ -f "${tmp_dir}/md2pdf_temp.pdf" ] && [ ! -f "${tmp_dir}/md2pdf.pdf" ]; then
        mv "${tmp_dir}/md2pdf_temp.pdf" "${tmp_dir}/md2pdf.pdf"
    fi
...
md2pdf::handle_output() {
    local tmp_dir="$1"
    local output="$2"
    local tmp_epub="${tmp_dir}/md2pdf.epub"
    local tmp_pdf="${tmp_dir}/md2pdf.pdf"
...
md2pdf() {
    ...
    md2pdf::parse_args "title" "author" "output" "$1" "$2" "$3"
    
    # Setup temporary directory - directly capture the output
    local tmp_dir=$(md2pdf::setup_temp_dir "md2pdf_tmp")
    
    # Debug paths to verify temporary directory is correctly set
    # md2pdf::debug_paths "$tmp_dir"
    
    # Setup cleanup trap (can be commented out for debugging)
    trap 'rm -rf "$tmp_dir"' EXIT
    
    # Process markdown from clipboard
    md2pdf::process_markdown "$tmp_dir"
    
    # Process links and create references section
    md2pdf::process_links "$tmp_dir"
    
    # Verify markdown file was created
    if [ ! -f "${tmp_dir}/md2pdf.md" ]; then
        echo "ERROR: Markdown file was not created at ${tmp_dir}/md2pdf.md"
        ls -la "$tmp_dir"
        return 1
    fi
    
    # Create LaTeX and PDF files - pass title and author
    md2pdf::create_latex_pdf "$tmp_dir" "$title" "$author"
    
    # Handle output and cleanup
    md2pdf::handle_output "$tmp_dir" "$output"
}
